...
const fetchJson = async <T = any>(url: string, options: FetchOptions = {}): Promise<T> => {
...
export async function listAvailableWorkspaces(token: string): Promise<string[]> {
...
const fetchBucketStats = async (
    ...
    if (workDir === 'raw_images') {
        stats.tiffs = data.objects.filter((obj: any) => /\.(tif|tiff|png|jpe?g)$/i.test(obj.name));
...
const getUploadUrl = async (
    ...
    const result = await fetchWithAuth<{ url: string }>(url, token, { method: 'PUT', headers: { 'Accept': 'application/json' } });
    return result.url;
};
...
export async function checkBucketExists(token: string, searchTerm: string): Promise<boolean> {
    try {
